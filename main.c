...
int main() {
    ...
    do {
        ...
        case 3:
            ...
            if (compareStr(str, "year")) {
                sortCars(list, sortYears);
            }
            if (compareStr(str, "cost")) {
    ...
    for (int i = 0; i < cars->size - 1; ++i) {
        for (int j = i + 1; j < cars->size; ++j) {
            if (!sortFunc(cars->cars+j - 1, cars->cars+j)) {
                printf("%s %s\n", cars->cars[j - 1].model, cars->cars[j].model);
...
int delCar(Cars* cars) {
    int count = 0,*l = (int*)mallocList(cars->size, sizeof(int));
    printf("\nBrand:");
    char* s = getStr();
    ...
    if (!count) {
        ...
        for (int i = 0; i < count; printf("\n%s", cars->cars[l[i]].model));
        printf("\nModel:");
        char* s = getStr();
        int i = 0;
        for (; i < cars->size && !compareStr(s, cars->cars[l[i]].model); ++i);
        if (i == cars->size) {
            printf("\nNot found");
            return !OK;
        }
        swapCars((Car**)cars->cars + i, (Car**)cars->cars + cars->size - 1);
...
int outputStruct(Car* car) {
    printf("%s:%s\n", VAR_NAME(brand), car->brand);
    printf("%s:%s\n", VAR_NAME(model), car ->model);
    printf("%s:%d\n", VAR_NAME(year), car->year);
    printf("%s:%d$\n", VAR_NAME(cost), car->cost);
    return OK;
}

void freeStruct(Car* car) {
    freeOneDStr(car->brand);
    freeOneDStr(car->model);
